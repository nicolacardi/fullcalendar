...
export class AppComponent implements OnInit, AfterViewInit{
  ...
  //calendarEvents: any;
  impostaData = new Date('2020-02-18');
  calendarEvents= 
    [
      {
        id: 1,
        ...
        color: 'yellow',
        allDay: true,
        start: this.impostaData,
        end: this.impostaData
      },
  ];
  ...
  dateClick(data) {
    //data.end è null se fullDay=true o se è stato impostato un evento nella weektimegrid o daytimegrid
    //solo in questo secondo caso imposto la fine a 1 h più dell'inizio
    //serve impostare un valore perchè per default non lo fa e questo in cascata determina problemi
    let startFullDate = new Date(data.date);
    let endFullDate = new Date(data.date);
    if (data.end == null && data.allDay == false)  {
      endFullDate.setTime(startFullDate.getTime()+3600000);
    } else {
      endFullDate = null;
    }
